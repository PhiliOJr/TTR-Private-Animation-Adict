...
class DistributedToonAI(DistributedPlayerAI.DistributedPlayerAI, DistributedSmoothNodeAI.DistributedSmoothNodeAI, PetLookerAI.PetLookerAI):
    ...
    def verifyDNA(self):
        ...
        if self.isPlayerControlled():
            ...
            if self.dna.getAnimal() == 'cat':
                allowedColors = allowedColors + [38]

            if 38 in [self.dna.legColor, self.dna.armColor, self.dna.headColor]: # Disney ALSO didn't do this. Verify that a toon is fully black/white.
                if self.dna.legColor != 38:
                    self.dna.legColor = 38
                    changed = True
                if self.dna.armColor != 38:
                    self.dna.armColor = 38
                    changed = True
                if self.dna.headColor != 38:
                    self.dna.headColor = 38
                    changed = True
    ...
    def setMaxNPCFriends(self, max):
        if max & 32768:
            ...
            max &= 32767
        configMax = config.GetInt('max-sos-cards', 8)
        if configMax != max:
    ...
    # Begin ToonFest Money (Tokens)

    def b_setMaxTokens(self, maxTokens):
        self.d_setMaxTokens(maxTokens)
        self.setMaxTokens(maxTokens)
        if self.getTokens() > maxTokens:
            self.b_setBankTokens(self.bankTokens + (self.getTokens() - maxTokens))
            self.b_setTokens(maxTokens)

    def d_setMaxTokens(self, maxTokens):
        self.sendUpdate('setMaxTokens', [maxTokens])
    ...
    def addTokens(self, deltaTokens):
        ...
        pocketTokens = min(tokens, self.maxTokens)
        self.b_setTokens(pocketTokens)

    def takeTokens(self, deltaTokens):
        totalTokens = self.tokens
        if deltaTokens > totalTokens:
            self.notify.warning('Not enough tokens! AvId: %s Has:%s Charged:%s' % (self.doId, totalTokens, deltaTokens))
            return False
    ...
    def setTokens(self, tokens):
        if tokens < 0:
            ...
            tokens = 0
            commentStr = 'User %s has negative tokens %s' % (self.doId, tokens)
            dislId = self.DISLid
            if config.GetBool('want-ban-negative-tokens', False):
                #simbase.air.banManager.ban(self.doId, dislId, commentStr)
                pass
        self.tokens = tokens
    ...
    def doResistanceEffect(self, msgIndex):
        ...
        if msgType == ResistanceChat.RESISTANCE_TOONUP:
            ...
            self.inventory.addUniteItem(msgValue, 5)
            self.d_setInventory(self.inventory.makeNetString())
    ...
    if simbase.wantKarts:
        ...
        def setKartDNA(self, kartDNA):
            self.b_setKartBodyType(kartDNA[KartDNA.bodyType.value])
            self.b_setKartBodyColor(kartDNA[KartDNA.bodyColor.value])
            self.b_setKartAccColor(kartDNA[KartDNA.accColor.value])
            self.b_setKartEngineBlockType(kartDNA[KartDNA.ebType.value])
            self.b_setKartSpoilerType(kartDNA[KartDNA.spType.value])
            self.b_setKartFrontWheelWellType(kartDNA[KartDNA.fwwType.value])
            self.b_setKartBackWheelWellType(kartDNA[KartDNA.bwwType.value])
            self.b_setKartRimType(kartDNA[KartDNA.rimsType.value])
            self.b_setKartDecalType(kartDNA[KartDNA.decalType.value])
        ...
        def updateKartDNAField(self, dnaField, fieldValue):
            if not checkKartFieldValidity(dnaField):
                ...
                return
            if dnaField == KartDNA.bodyType.value:
                    ...
                    if fieldValue == InvalidEntry:
                        ...
                        if fieldValue not in self.accessories:
                            if fieldValue != getDefaultColor():
                                ...
                                return
                        if getAccessoryType(fieldValue) != KartDNA.bodyColor.value:
        ...
        def __updateKartDNAField(self, dnaField, fieldValue):
            if dnaField == KartDNA.bodyColor.value:
                self.b_setKartBodyColor(fieldValue)
            elif dnaField == KartDNA.accColor.value:
                self.b_setKartAccessoryColor(fieldValue)
            elif dnaField == KartDNA.ebType.value:
                self.b_setKartEngineBlockType(fieldValue)
            elif dnaField == KartDNA.spType.value:
                self.b_setKartSpoilerType(fieldValue)
            elif dnaField == KartDNA.fwwType.value:
                self.b_setKartFrontWheelWellType(fieldValue)
            elif dnaField == KartDNA.bwwType.value:
                self.b_setKartBackWheelWellType(fieldValue)
            elif dnaField == KartDNA.rimsType.value:
                self.b_setKartRimType(fieldValue)
            elif dnaField == KartDNA.decalType.value:
...
def dna(part, value):
    ...
    def isValidColor(colorIndex):
        if not 0 <= colorIndex <= 38: # This could actually be selected from ToonDNA, but I prefer this :D
            return False
        if colorIndex == 0 and dna.getAnimal() != 'bear':
            return False
        if colorIndex == 38 and dna.getAnimal() != 'cat':
            return False
        return True
...
def dump_doId2do():
    ...
    sorted_objSizes = sorted(iter(objSizes.items()), key=operator.itemgetter(1))
...
def phrase(phraseStringOrId):
    ...
    try:
        ...
        for scId, string in strings.items():
